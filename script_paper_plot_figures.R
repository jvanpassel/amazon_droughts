...
tac_slope_drivers<-mask(tac_slope_drivers,nc_p,updatevalue=0,inverse=T)
tac_slope_drivers<-mask(tac_slope_drivers,ne_p,updatevalue=2,inverse=T)
tac_slope_drivers<-mask(tac_slope_drivers,sw_p,updatevalue=3,inverse=T)
tac_slope_drivers<-mask(tac_slope_drivers,sc_p,updatevalue=0,inverse=T)
tac_slope_drivers<-mask(tac_slope_drivers,se_p,updatevalue=4,inverse=T)
...
ggplot() +  
  ...
  scale_fill_manual(values = c('grey50','darkred','steelblue4','darkorange2','darkgreen'),
                    name='Drought-related drivers \nof slowing down',
                    na.value = 'transparent',na.translate = F,
                    labels=c('No unambiguous drivers','Frequency',
                             'Intensity, duration',
                             'Frequency, intensity',
                             'Frequency, duration')) +
  geom_map(data = worldmap,map = worldmap,aes(x=long,y=lat,map_id=id),fill=NA,color='grey30') +
