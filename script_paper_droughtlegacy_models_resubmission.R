...
#Create model for woody plant subregions: slope 01-19 (60m) ~ drought 01-19 + climate####
tac_slope<-raster('evi_01_19_lag1_60m_slope.tif')
tac_slope_nona<-read.csv('evi_01_19_lag1_60m_nonaslope_sw13.csv')[,-1]
droughtlegacy_df<-read.csv('droughtlegacy_01_19_nonaslope.csv')[,-1]
droughtlegacy_df[is.na(droughtlegacy_df)]<-0
droughtlegacy_clim_df<-cbind(droughtlegacy_df,seas_df,interannvar_df)

subregions<-shapefile('./Silva-Souza and Souza, 2020, Journal of ecology/amazon_subregions.shp')
subregions_crs<-spTransform(subregions,crs(amazon))
subregion_i<-subregions_crs[subregions_crs$Subregion==1,] #change for each subregion

#extract cell numbers with non-NA slope values and extract coordinates
tac_slope_subregion_cellnumbers<-(raster::extract(tac_slope,subregion_i,cellnumbers=T))[[1]]
tac_slope_subregion_cellnumbers<-tac_slope_subregion_cellnumbers[!is.na(tac_slope_subregion_cellnumbers[,2]),1]
tac_slope_subregion_coord<-xyFromCell(tac_slope,tac_slope_subregion_cellnumbers)
tac_slope_subregion_coord<-as.data.frame(tac_slope_subregion_coord)
tac_slope_nona_coord<-as.data.frame(tac_slope_nona[,1:2])

#get matching row numbers with input data and extract values
rownumbers_match<-match(do.call("paste", tac_slope_subregion_coord), do.call("paste", tac_slope_nona_coord))
tac_slope_nona_subr<-tac_slope_nona[rownumbers_match,]
droughtlegacy_char_15_19_df_subr<-droughtlegacy_char_15_19_df[rownumbers_match,]
droughtlegacy_char_15_19_df_subr$MAP<-raster::extract(MAP,tac_slope_nona_subr[,1:2])

#create dataset for regional model
droughtlegacy_clim_df_st<-scale(droughtlegacy_char_15_19_df_subr[,3:21])
model_var_01_19<-cbind(tac_slope_nona_subr,droughtlegacy_clim_df_st)
colnames(model_var_01_19)[c(3)]<-c('slope')

#select 1200 random pixels 
set.seed(1)
model_var_01_19<-model_var_01_19[sample(nrow(model_var_01_19), 1200), ]
...
colMeans(droughtlegacy_char_15_19_df_subr[,3:8],na.rm=T)
apply(droughtlegacy_char_15_19_df_subr[,3:8],2,function(x){sd(x,na.rm=T)})
